...
pub fn border_style(focused: bool) -> Style {
    if focused {
        Style::default().fg(flexoki::CYAN)
    } else {
        Style::default().fg(flexoki::TX_3)
    }
}
...
pub fn inactive_tab_style() -> Style {
    Style::default().fg(flexoki::TX_2).bg(flexoki::UI)
}
...
mod tests {
    ...
    fn test_border_style_focused() {
        ...
        assert_eq!(style.fg, Some(flexoki::CYAN));
    }
    ...
    fn test_inactive_tab_style() {
        ...
        assert_eq!(style.fg, Some(flexoki::TX_2));
        assert_eq!(style.bg, Some(flexoki::UI));
    }
}
